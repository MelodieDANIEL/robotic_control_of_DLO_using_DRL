...
class PandaFriteEnv(gym.Env):
	...
	def __init__(self, database = None, distance_threshold = None, gui = None):
		...
		self.id_frite_to_follow = [53, 179]
	...
	def set_gym_spaces(self):
		...
		# observation = 18 float -> see function _get_obs
		self.observation_space = spaces.Box(np.finfo(np.float32).min, np.finfo(np.float32).max, shape=(18,), dtype=np.float32)
	...
	def get_obs(self):
		...
		# self.goal = len(self.id_frite_to_follow = [53, 179]) x 3 values (x,y,z) cartesian world position = 6 floats
		# observation = 
		#  3 floats (x,y,z) gripper link cartesian world position  [0,1,2]
		# + 3 float (vx, vy, vz) gripper link cartesian world velocity [3,4,5]
		# + current cartesian world position of id frite to follow (6 floats) [6,7,8,9,10,11]
		# + self.goal cartesian world position of id frite to reach (6 floats) [12,13,14,15,16,17]
